...
def get_all_evals(cfg, model, tokenizer, eval_dataloader):
    ...
    for batch in tqdm(eval_dataloader):
        ...
        eval_logs['gt_loss_per_token'] = eval_logs.get('gt_loss_per_token', []) + (
...
def run_generation(cfg, batch, model, tokenizer):
    ...
    if cfg.model_family == "llama3-8b-instruct":
        input_strings = tokenizer.batch_decode(input_ids,
                                               skip_special_tokens=False)  # skip special token was TRUE for llama2b
